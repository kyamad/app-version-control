...
当社が公開している **54 本の iOS アプリ** について、以下の情報をまとめて管理します。
...
> 通常は App Store に新バージョンが並べば、Apple の API に反映され次第（通常は数時間以内）、直後の毎時実行で `version.json` へ自動反映されます。
...
`version.json` は 1 ファイルに 54 本すべてが入っているため、同時編集はコンフリクトの原因になります。更新作業は「取得 → 編集 → すぐ push」を短時間で行うことを推奨します。
...
- JSON の **カンマ（`,`）** と **ダブルクォート（`"`）** を壊さないでください。1 文字でも欠けると JSON 全体が読み込めなくなり、全 54 本のアプリに影響します。
...
- **1時間ごと**に自動実行（毎時17分）
- 毎時00分は GitHub 全体で実行が集中して待たされやすいため、あえて17分にずらしています。それでも混雑時は予定時刻から数十分遅れることがあります
- 手動実行時に `dry_run` を `true` にすると、**差分の確認だけ行い commit しません**。挙動を確かめたいときに使ってください

### 手動で今すぐ反映したいとき

**Actions** タブ →「Sync app versions from App Store」→ 右上の **Run workflow** から実行してください。

⚠️ 過去の実行を開いて **「Re-run all jobs」で再実行するのは避けてください。** 再実行は、その実行が作られた時点の**古いコミット**をチェックアウトします。それ以降に他のメンバーのコミットが入っていると、push が拒否されて失敗します。**Run workflow** なら常に最新の `main` で実行されます。

### App Store に並んでから反映されるまでのタイムラグ

このワークフローが参照している Apple の Lookup API は CDN 経由で配信されているため、**App Store に新バージョンが並んでから API に反映されるまで数時間かかることがあります**（2026年8月に約3時間45分の遅延を実測しています）。

そのため、**リリース直後に手動実行しても「変更なし」と判定されることがあります。これは異常ではありません。**API 側に反映されれば、次の毎時実行で自動的に取り込まれます。急ぎでなければそのままお待ちください。
...
> App Store に新バージョンが並ぶ → **Apple の API に反映され次第、直後の毎時実行で自動反映** → **旧バージョンの利用者は起動時に強制アップデート対象になる**
